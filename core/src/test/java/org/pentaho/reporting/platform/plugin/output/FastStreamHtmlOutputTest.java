...
public class FastStreamHtmlOutputTest {
  FastStreamHtmlOutput fastStreamHtmlOutput;
  private IAsyncReportListener listener;

  @Before public void setUp() {
    fastStreamHtmlOutput = new FastStreamHtmlOutput( "" ); //$NON-NLS-1$
  ...
  @Test public void testPaginate() throws Exception {
    Assert.assertEquals( 0, fastStreamHtmlOutput.paginate( null, 0 ) );
  }

  @Test public void testSupportsPagination() throws Exception {
    Assert.assertEquals( false, fastStreamHtmlOutput.supportsPagination() );
  }

  @Test public void testGetReportLock() throws Exception {
    Assert.assertEquals( fastStreamHtmlOutput, fastStreamHtmlOutput.getReportLock() );
  }

  @Test
  public void testGenerateListener() throws Exception {
    ClassicEngineBoot.getInstance().start();
    fastStreamHtmlOutput.generate( new MasterReport(), 1, new ByteArrayOutputStream(), 1 );
  ...
  public void testGenerateNotFast() throws Exception {
    try {
      ...
      fastStreamHtmlOutput.generate( report, 1, new ByteArrayOutputStream(), 1 );
