 ...
 */
   public class MockPluginResourceLoader implements IPluginResourceLoader {
       ...
       public InputStream getResourceAsStream(ClassLoader classLoader, String s) {
           try  {
                 File file = new File("");
                 return new FileInputStream(new File(file.getAbsolutePath() + (file.getAbsolutePath().endsWith("reporting-plugin") ? "": "/platform-plugins/reporting-plugin") + "/tests/integration-tests/resource/solution/system/reporting" + s));
