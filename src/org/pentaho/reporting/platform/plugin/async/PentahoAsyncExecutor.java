...
public class PentahoAsyncExecutor implements ILogoutListener, IPentahoSystemListener, IPentahoAsyncExecutor {
  ...
  private Map<CompositeKey, ListenableFuture<InputStream>> futures = new ConcurrentHashMap<>();
  ...
  @Override public UUID addTask( final IAsyncReportExecution<InputStream> task, final IPentahoSession session ) {
    ...
    final ListenableFuture<InputStream> result = executorService.submit( task );
  ...
  @Override public void schedule( final UUID id, final IPentahoSession session ) {
    ...
    final IAsyncReportExecution<InputStream> runningTask = tasks.get( compositeKey );
    if ( runningTask != null ) {
      if ( runningTask.schedule() ) {
        final ListenableFuture<InputStream> future = futures.get( compositeKey );
        Futures.addCallback( future, new FutureCallback<InputStream>() {
          @Override public void onSuccess( final InputStream result ) {
            try {
              if ( session != null && !StringUtil.isEmpty( session.getName() ) ) {
                SecurityHelper.getInstance()
                  .runAsUser( session.getName(), new WriteToJcrTask( id, session, runningTask, result ) );
              }
  ...
  @Override public void onLogout( final IPentahoSession iPentahoSession ) {
    ...
    for ( final Map.Entry<CompositeKey, ListenableFuture<InputStream>> entry : futures.entrySet() ) {
      if ( ObjectUtils.equals( entry.getKey().getSessionId(), iPentahoSession.getId() ) ) {

        final IAsyncReportExecution<InputStream> task = tasks.get( entry.getKey() );

        final ListenableFuture<InputStream> value = entry.getValue();
        ...
        Futures.addCallback( value, new FutureCallback<InputStream>() {
          @Override public void onSuccess( final InputStream result ) {
            if ( result != null ) {
    ...
    if ( !hasScheduled ) {
      ...
      final File sessionStagingContent = getStagingDir( iPentahoSession );

      if ( sessionStagingContent == null ) {
        return;
      }
  ...
  private class WriteToJcrTask implements Callable<Object> {
    ...
    private final IAsyncReportExecution<InputStream> parentTask;
    private final InputStream inputStream;

    WriteToJcrTask( final UUID id, final IPentahoSession session,
                    final IAsyncReportExecution<InputStream> parentTask,
