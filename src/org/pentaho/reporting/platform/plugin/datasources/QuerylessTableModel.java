...
public class QuerylessTableModel implements CloseableTableModel, MetaTableModel {
  ...
  public void setQuery(Query query) {
    ...
    for(int column=0; column<query.getSelections().size(); column++) {
      ...
      (column, MetaAttributeNames.Formatting.NAMESPACE, MetaAttributeNames.Formatting.LABEL, name);
      Class<?> clazz = this.getColumnClass(column);
      if(clazz == Float.class) {
        metaData.setColumnAttribute
        (column, MetaAttributeNames.Numeric.NAMESPACE, MetaAttributeNames.Numeric.CURRENCY, false); 
        metaData.setColumnAttribute
        (column, MetaAttributeNames.Numeric.NAMESPACE, MetaAttributeNames.Numeric.SCALE, 0); 
        metaData.setColumnAttribute
        (column, MetaAttributeNames.Numeric.NAMESPACE, MetaAttributeNames.Numeric.SIGNED, false); 
        metaData.setColumnAttribute
        (column, MetaAttributeNames.Numeric.NAMESPACE, MetaAttributeNames.Numeric.PRECISION, 2147483647); 
      }
    }
  }
  ...
  public Object getValueAt(int row, int column) {
    ...
    return Messages.getInstance().getString(DEFAULT_STRING_KEY_FOR_ROW + String.valueOf(row), String.valueOf(column));
  }
